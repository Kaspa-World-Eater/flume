 ...
 *
 * A metered session is between two public keys, so both sides need a stable one: a broadcaster that
 * generated a fresh key on every start would be a different seller to every buyer, and a listener
...
const HOME = join(homedir(), '.flume');

export type Role = 'listener' | 'broadcaster';
