...
export async function claim(sellerSk: string, covenantId: string, voucher: Voucher, claimSompi: bigint, feeSompi = CLAIM_FEE): Promise<{ txid: string; paid: bigint; channel: Channel }> {
  const record = recall(covenantId);
  const tooSmall = claimTooSmall(record.channel, claimSompi, feeSompi);
